...
function createGeneratorConfig (options) {
  const generatorConfig = {
    ...
    topRef: true,
    jsDoc: 'extended',
    // sortProps: options.sortProperties,
    strictTuples: false,
...
function modifySchema (schema, options) {
  const { expandTypes, removeProps, detectGraphQL, graphQLExpandedTypes } = options
...
function convertTypeScript (options) {
  const definitions = []
          ...
          new Transform({
            ...
            transform (chunk, encoding, callback) {
              const file = new Vinyl({
                ...
                contents: Buffer.from(convertTypeScript({ ...options, path: chunk.basename }), encoding)
              })
