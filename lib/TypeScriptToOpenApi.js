...
function createGeneratorConfig (path, options) {
  const generatorConfig = {
    ...
    sortProps: true,
    strictTuples: false,
    skipTypeCheck: typeof options.skipTypeCheck === undefined ? true : options.skipTypeCheck,
    extraTags: []
  }
...
function modifySchema (schema, options) {
  const { expandTypes = [], removeProps = [], detectGraphQL = true, graphQLExpandedTypes = ['Maybe', 'Scalar'] } = options
...
function typeScriptToOpenApi (options) {
  return new Promise((resolve, reject) => {
    try {
      src(options.typeScript, { buffer: false })
        .pipe(
          new Transform({
            objectMode: true,
            transform (chunk, encoding, callback) {
              const file = new Vinyl({
                path: './' + PROJECT_ASSETS + '/' + chunk.basename.replace(/(.d.ts$|.ts$)/gu, '.components.yaml'),
                contents: Buffer.from(convertTypeScript(chunk.basename, options), encoding)
              })
        ...
        .pipe(dest('./' + PROJECT_ASSETS + '/typescript'))
        .on('end', () => resolve())
